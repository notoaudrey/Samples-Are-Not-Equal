...
class ResNet(nn.Module):
    ...
    def forward(self, x ,is_dropout=False,is_norm=False):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
...
def resnet18_cifar(**kwargs):
    # return {'backbone': ResNet(BasicBlock, [2, 2, 2, 2], **kwargs), 'inplanes': 512}
    return ResNet(BasicBlock, [2, 2, 2, 2], **kwargs)
    # return {'backbone': ResNet(BasicBlock, [2, 2, 2, 2], **kwargs), 'dim': 512}

# def ResNet18Control(feature_dim=512):
#     return {'backbone': ResNetControl(BasicBlock, [2, 2, 2, 2], **kwargs), 'dim': 512}
