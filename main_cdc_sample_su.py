...
FLAGS.add_argument('--config_exp', default='scripts/cdc/stl/cdc_seed5.yaml', help='Location of experiments config file')
...
def main():
    ...
    indices_per_bin=initialize_weights_bias(cfg, model, cali_mlp, features, val_dataloader, alpha=alpha)    
    predictions, features = get_predictions(cfg, val_dataloader, model, return_features=True)
    ...
    for epoch in range(start_epoch, cfg['max_epochs']):
        ...
        if (epoch+1) % 1 == 0:
            ...
            predictions = get_predictions_su(cfg, sample_dataloader, model, cali_mlp = cali_mlp, tracker=tracker)
            clustering_stats = calibration_evaluate(cfg, cfg['cdc_dir'], epoch, 0, predictions,
                                                title=cfg['cluster_eval']['plot_title'],
                                                compute_confusion_matrix=False, remove = tracker.removed, bins = indices_per_bin)
